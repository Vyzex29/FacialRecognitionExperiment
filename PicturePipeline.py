    ...
    for root, dirs, dirfiles in os.walk(config.paths['COLLECTED_IMAGES_PATH']):
        for file in dirfiles:
            if file.endswith("png") or file.endswith("jpg"):
                ...
                saveDirectory = os.path.join(config.paths['TENSOR_IMAGES_PATH'],os.path.basename(root))
                if not os.path.exists(saveDirectory):
                    os.mkdir(saveDirectory)
                saveImage = final_image.copy()  # overwrite original pic with the modified one
                savePath = os.path.join(saveDirectory, file)
                ...
                if detectionBoxCount > 1:
                    ...
                    writeDetectionBoxXMLForImage(saveDirectory, os.path.abspath(file), file, w, h, selectedBoxList)
                    continue

                writeDetectionBoxXMLForImage(saveDirectory, os.path.abspath(file), file, w, h, boxList)
                while True:
                    cv2.imshow(file, final_image_array)
